...
public class StatementGraphController {
	...
	public String build(@RequestBody UserInput input) throws AtomSetException, IteratorException, ChaseException, HomomorphismException {
		DefeasibleKnowledgeBase kb = new DefeasibleKnowledgeBase();
		kb.add(input.getDlgp());
		StatementGraph sg = new StatementGraph(kb);
		sg.build();
		return sg.toViewJSON();
	}
	
	@CrossOrigin(origins = origins)
	@RequestMapping(path = "/api/sg/query", method = RequestMethod.POST)
	public String query(@RequestBody UserInput input) throws AtomSetException, IteratorException, ChaseException, HomomorphismException {
